// @ts-ignore
/* eslint-disable */
// API 更新时间：
// API 唯一标识：
import * as aiChatMessageController from "./aiChatMessageController";
import * as aiChatSessionController from "./aiChatSessionController";
import * as aiModelController from "./aiModelController";
import * as aiPlatformController from "./aiPlatformController";
import * as alipayController from "./alipayController";
import * as basicErrorController from "./basicErrorController";
import * as commentController from "./commentController";
import * as fileController from "./fileController";
import * as homeController from "./homeController";
import * as noticeController from "./noticeController";
import * as orderController from "./orderController";
import * as postController from "./postController";
import * as productController from "./productController";
import * as scoreController from "./scoreController";
import * as taskController from "./taskController";
import * as userController from "./userController";
import * as webSiteController from "./webSiteController";
export default {
  aiChatMessageController,
  aiChatSessionController,
  aiModelController,
  aiPlatformController,
  alipayController,
  basicErrorController,
  commentController,
  fileController,
  homeController,
  noticeController,
  orderController,
  postController,
  productController,
  scoreController,
  taskController,
  userController,
  webSiteController,
};
